...
public class DaoDados {
    ...
    private Integer tentativas = 5;
    private Boolean loginPermition = false;
    private Integer empresaUser;

    public DaoDados(Looca looca, Sistema sistema, Processador processador, Temperatura temp, Memoria memoria, String hostNameUser, String ipUser, String ipServidor, Integer fkEmpresa, Integer fkEmpresaServer, Integer fkDataCenter, Integer fkDataCenterServer, Integer idComponente, Integer idComponenteServer, Integer emitirAlerta, Integer tentativasAcesso, Integer tipoLogin, Integer tentativas, Boolean loginPermition, Integer empresaUser) {
        this.looca = looca;
        ...
        this.loginPermition = loginPermition;
    ...
    public void verificarLogin(Integer opcao, String email, String senha) {
        ...
        if (tentativas.equals(0)) {
            System.out.println("\033[1;31mLimite de tentativas atingido!\033[m tente novamente mais tarde.");
            String descricao = """
                    : Usuário do IP %s, hostName: %s. Esgotou o número de tentativas de login""".formatted(ipUser, hostNameUser);
            setLog(descricao);
            System.exit(0);
        } else {
            switch (opcao) {
                case 1: {
                    Integer verificacao = conServer.queryForObject("SELECT COUNT(*) FROM Usuario where email = ? and senha = ?", Integer.class, email, senha);
                    if (verificacao == 0) {
                        tentativas--;
                        System.out.println("""
                                    Email ou senha \033[1;31minválido!\033[m Você tem \033[1;31m%d\033[m tentativas""".formatted(tentativas));
                        String descricao = """
                    : Usuário do IP %s, hostName: %s. Tentou fazer login como usuário e não conseguiu. ainda tem %s tentativas""".formatted(ipUser, hostNameUser, tentativas);
                        setLog(descricao);
                    } else {
                        String verificacao2 = conServer.queryForObject("SELECT nome FROM Usuario where email = ? and senha = ?", String.class, email, senha);
                        empresaUser = conServer.queryForObject("SELECT fkEmpresa FROM Usuario where email = ? and senha = ?", Integer.class, email, senha);
                        loginPermition = true;
                        tipoLogin = 1;
                        String descricao = """
                    : Usuário do IP %s, hostName: %s. Realizou login como usuário %s!""".formatted(ipUser, hostNameUser, verificacao2);
                        setLog(descricao);
                        System.out.println("""
                    +----------------------------------------------+
                    |   Bem-vindo, %s ao \033[1;34mPerformee.\033[m     """.formatted(verificacao2));
                    }
                    break;
                }
                case 2: {
                    Integer verificacao = conServer.queryForObject("SELECT COUNT(*) FROM Administrador where email = ? and senha = ?", Integer.class, email, senha);
                    if (verificacao == 0) {
                        tentativas--;
                        System.out.println("""
                                    Email ou senha \033[1;31minválido!\033[m Você tem \033[1;31m%d\033[m tentativas""".formatted(tentativas));
                        String descricao = """
                    : Usuário do IP %s, hostName: %s. Tentou fazer login como Administrador e não conseguiu. ainda tem %s tentativas""".formatted(ipUser, hostNameUser, tentativas);
                        setLog(descricao);
                    } else {
                        String verificacao2 = conServer.queryForObject("SELECT nome FROM Administrador where email = ? and senha = ?", String.class, email, senha);
                        loginPermition = true;
                        tipoLogin = 2;
                        String descricao = """
                    : Usuário do IP %s, hostName: %s. Realizou login como Administrador %s!""".formatted(ipUser, hostNameUser, verificacao2);
                        setLog(descricao);
                        System.out.println("""
                    +----------------------------------------------+
                    |   Bem-vindo, %s ao \033[1;34mPerformee.\033[m     """.formatted(verificacao2));
    ...
    public Boolean buscarIp(String ipServer) {
        ...
        Integer verificacaoIpServer;

        if (tipoLogin.equals(1)) {
            verificacaoIpServer = conServer.queryForObject("SELECT COUNT(*) FROM Servidor where IpServidor = ? and fkEmpresa = ?", Integer.class, ipServer, empresaUser);
        } else {
            verificacaoIpServer = conServer.queryForObject("SELECT COUNT(*) FROM Servidor where IpServidor = ?", Integer.class, ipServer);
        }
    ...
    public Boolean getLoginPermition() {
        return loginPermition;
    }

    public void setLoginPermition(Boolean loginPermition) {
        this.loginPermition = loginPermition;
    }

    public Integer getEmpresaUser() {
        return empresaUser;
    }

    public void setEmpresaUser(Integer empresaUser) {
        this.empresaUser = empresaUser;
    }
